...
当前日期安排以 [C4 执行单](C4_EXECUTION_PLAN.md) 为准，截止周日 2026-09-13；下文相对
日程仅保留作原估计。现有 M1 为 200 updates，新增 M2 优化前核对历史总账与实际联合
smoke 成本，检查同 T 可行性。资源上限不变；C5 可按学校项目范围预登记 1–2 roots，
现场准备与失败同样占用既有最多 18 attempts / 4 小时 CARLA 预算。
...
现有 M1 已完成 200 updates；C4 复用其实际配方，先依据真实联合 smoke 核对共同步数
在剩余预算内可完成，不为重新冻结配方重复执行 M1。
...
不承诺零算力开销。本轮在正式 M2 前测出真实联合成本，核对既有 T=200 与累计预算。
