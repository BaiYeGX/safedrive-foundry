...
## 学校项目交付优先（2026-09-11 用户确认）

当前目标是限时完成可运行、可复现、可展示的学校项目，优先形成相对原始模型 M0 的明确、
真实优势。允许调整交付范围、评价主次、实现复杂度和案例呈现；论文级完整验证作为后续
改进，不再要求本轮补齐所有场景、所有指标或证明普遍驾驶能力。
本节是当前交付优先级；与下文旧论文规划及其他活动文档冲突时，以本节为准。
指标的单位、实际测量、模型身份和已冻结实验记录仍按原定义保存。

**截止：本周日 2026-09-13。** 周五优先 C3 收尾与 CARLA/M1，周六完成 C4 和现场接入，
周日中午冻结演示版本，下午用于排练、材料与 GitHub 同步。详见紧急收尾方案的日期表。

**用户补充硬要求：老师必须看实时 CARLA 闭环演示。** 离线数据范围可以缩小，但现场
闭环不能用录像、日志重放或离线动画替代。先用现有 M1 打通实际感知—模型推理—控制—
车辆反馈，再完成 C4 并接入 M2/新 World。演示场景数量、画质和统计矩阵可以缩减，
真实控制来源、模型身份和现场运行必须记录；优先级详见紧急收尾方案。

用户允许不影响结论有效性的工程取巧，具体执行如下：

- 优先复用旧数据、原生规划重建、逐头部分监督、已有模型与工具；缓存不变的预处理结果，
  只做解决实际错误所需的验证。真实 batch、有效更新和可重载产物仍需实际完成。
- C3 离线实验接受有限支持距离、单 seed、固定小预算；缺少完整 19 m、非零初速覆盖、
  P-FDE/P-SPEED 或未采满补采上限，本身不再阻止 C3 作为离线基线进入 C4。
  补采上限不是完成配额；只有下一步缺少可用的真实标签时才考虑必要补采。
- 可以将已有真实优势放在正文前面，选择讲得清楚或改善明显的真实案例，并标注选取理由；
  案例展示与全量汇总分开，完整表保留其他指标、退化、简单基线和实际覆盖范围。
- 可以追加有明确用途的场景/距离子集分析。已看过结果后确定的子集或展示主次明确标为
  事后开发分析；双方使用同一批 root、真值 mask 和分母，原全量结果保留。
- 允许在工程完成前提下，以某个明确任务的真实改善作为项目亮点，不要求所有指标获胜、
  所有 root 改善、CI 显著或击败每一种简单方法。方法收益、原模型适配收益和数据工程
  成果分别说明，不把它们相加成一个“综合驾驶能力提升百分比”。

这种调整用于节约时间和准确突出优势。原始标签、失败记录、保留集隔离、真实训练及
候选—后果对应关系保持可追溯；改变监督或预测方法时另记版本。训练集增强不增加独立
root 数；使用 validation 调整的结果按开发结果解释。CARLA READY、唯一 tick owner 和
RESOURCES 的实际资源上限继续有效，未知耗时不按零计算。

### 当前展示与下一实验口径（school_delivery_v2）

| 层次 | 当前优先内容 | 解释范围 |
|---|---|---|
| C3 结题展示 | M1/M0 的 P-WP 时间 waypoint 位置误差优先，P-ADE 有效路线误差并列 | 现有 53-root 开发集上的原模型适配；本次为看到 C3 结果后的展示调整 |
| C4 策略实验 | M2/M1 的 P-WP 为下一次正式实验的主指标，P-ADE、P-VALID/P-FAIL 同表报告 | 在 M2 前冻结为新版本；路线退化单列，不把 P-WP 改善写成全面驾驶改善 |
| C4 后果实验 | b+R/b 的四头误差、配对差分与真实共享梯度 | 每头分别解释；某头改善可单列为亮点，不替代策略或闭环收益 |
| 工程与数据成果 | 旧数据利用、可重载权重、真实更新、完整预测与可访问案例 | 按实物和数量说明，不冒充额外模型性能指标 |

P-WP 单位是米，表示 0.25…2.5 s 的原生时间 waypoint 位置误差，不是 m/s 速度误差。
M1/M0 展示适配价值；M2/M1 才能说明新增 World 辅助的作用。当前强简单基线保留在对照表，
主展示无需以超过所有简单基线为交付门槛。
旧 c3_c6_metrics_v1 的公式和 C3 冻结结果保留；本节改变展示/后续实验主次，不回写旧
metrics-spec 或把这次选择说成在 C3 训练前登记。C4 开跑前将 school_delivery_v2 写入新
run config 和 metrics-spec，固定输入、mask、指标与比较。原 2%/3% 幅度继续作为期望值，
不作为结题通过门槛，也不为追逐它增加无界重训。

### 当前推进决定

当前 C3 全量指标与呈现见 [学校项目结果页](C3_SCHOOL_RESULTS.md)：P-WP 下降 84.52%，
52/53 roots 改善，route ADE 平均下降 35.97%；全部验证样本保留。独立评估的已知有效率、
单位和基线支持口径错误已修正并验证；资源历史总账与实际闭环仍单列待办。

紧急收尾与后续压缩范围见 [C3/C4 收尾方案](C3_C4_FINISH_PLAN.md)。最新分组复算在不
删除验证样本时，P-WP 均值已在九个场景类别及三个已有地图分组全部下降；优先用该全量
结果展示优势。因模型表现筛选的子集只能按明确 n/53 的事后分析展示，不能替换全量结论。

C3 保留为可用的有限范围离线基线，保留现有 M1，不为补满场景和展示指标重新训练。
实时 CARLA 是新增确认的硬性交付要求；下一步前置 CARLA 故障定位与 M1 最小现场闭环，
不能把启动与在线模型接入风险推迟到 M2 完成后。
C4 的必要前置工作集中为：核验候选与真实后果绑定并跑正确的联合 batch；复用已修正的
独立评估；补齐已知 smoke/失败资源消耗并确认剩余预算。
其他不影响当前实验有效性的完整性检查可后置并列为限制。若发现输入、监督或训练本身
影响结果有效性的错误，则处理受影响部分，不能靠展示调整消除错误。
M2 从原始 M0 开始，沿用 M1 的 SFT 数据、seed、root 顺序与 200 次更新作为比较基准。
若实际需要变更，另记配置并说明比较条件；不同预算不称同预算实验。
...
C3 当前为可用离线基线，算法结果已测量；此前完整验收声明按本次复核收窄，C4 尚未启动。当前
...
具体合同见 C3 执行文档与 WORLD_MODEL；C3 已有真实训练和可复算误差，独立有效率计算
已修正；资源记账和正式联合 smoke 仍待完成，不能把 C3 的适配收益冒充 C4 方法收益。
...
## 原正向结果预登记口径（v1 留档；当前主次见 school_delivery_v2）

本次以提高一次固定实验的可信收益机会为目标，方案和指标在 M1/M2 正式训练前冻结；
没有任何论文保证本项目正收益。以下是原 v1 主次，用于解释已经实施的 C3 实验；
2026-09-11 的结题展示和未运行 C4 主次已在上方显式调整，旧实验记录不回写。
...
本节保留 C3–C6 指标计算定义；C3 已随 run config 冻结并保存版本/hash。当前展示与 C4
主次由 school_delivery_v2 覆盖，后续实施时须在各自 run 中保存实际版本/hash。
