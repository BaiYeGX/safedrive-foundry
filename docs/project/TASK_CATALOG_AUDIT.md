...
| G1 | 9 | G1-01、G1-02 `COMPLETED`；G1-03～G1-09 `PENDING` |
| G2 | 6 | 6 项 `PENDING` |
| G3 | 7 | 7 项 `PENDING` |
| G4 | 6 | 6 项 `PENDING` |
| G5 | 6 | 6 项 `PENDING` |
...
- `PROGRESS.md`、当前任务 G1-02 和活动任务文件状态一致：G0 冻结完成，G1-01/G1-02 完成，G1-03 及后续任务保持 `PENDING`；推荐 G1-03 但未启动。
...
- G0 历史 `172.30.80.1` 未修改；后续任务文档不再将其作为运行时固定地址。
