...
**状态**：MEASURED_WITH_LIMITS / NOT_VERIFIED
**依赖**：G3-02
**阶段角色**：必做（**全项目最关键工程门**）
...
**2026-07-14 neural 前向 MEASURED（后被复审降级）**
- F0 报告：`docs/architecture/evidence/g3-03/f0_neural/report.json` — 加载/前向/determinism 有测
  - load DrivingModel + state_dict missing=0；~957M params
  - peak VRAM ≈2222 MB；latency **n=6**，P50≈125ms P95≈133ms
  - determinism max_diff=0；source=`neural_simlingo`
- 代码：`driving_vla/model/simlingo_runtime.py`、`neural_policy.py`
- 旧 fingerprint F0 见 `f0/SUPERSEDED.md`（仍作废）

**2026-07-16 验收复审 → MEASURED_WITH_LIMITS / NOT_VERIFIED**
保留：真 checkpoint 加载与 BF16 前向。
缺口（完整 F0 未 VERIFIED）：
1. 时延仅 6 样本；缺 20–100 step resource smoke。
2. 无 ≥30 min（或等价 step）稳定性记录。
3. 无 save/restore 证据。
4. checkpoint SHA256 仍 deferred，lineage 未冻结。
5. CandidateSet 时间为 `0..2.25s`，合同 2.5s；adapter 显式忽略 horizon 不一致。
不得标 COMPLETED / COMPLETED_WITH_LIMITS，直至 F0 清单全项有实测且合同一致。
