...
- `driving_vla/adapter` + `baselines`（Route/Ego、Vision K1、K2 接口）
- 测试：`tests/g3/test_g3_02_baselines.py`

**2026-07-16 验收复审 → IMPLEMENTED / NOT_VERIFIED**
缺口：
1. 无 ADE/FDE/覆盖率/可执行率/Safety 拒绝率/资源报告。
2. 所谓视觉基线为单图均值缩放，不是时序视觉基线。
3. 不满足“固定 split 上可评测对照标尺”的最小通过口径。
