...
## 2026-09-11 — 学校项目交付优先与 C3 当前定位

最新入口整理：C3 按学校项目的离线基线范围收尾，后续复用已有 M1；完整资源核对仍为
新增 GPU 优化前置。新增 [C4 执行单](docs/C4_EXECUTION_PLAN.md)，精简 START_TASK，
同步 WORLD_MODEL/ROADMAP 的主指标、200-step 对齐、现场前置与周日交付安排。
完整 M2 CLI 和真实联合运行明确标为待实现/NOT_RUN，本轮只修改文档；训练和现场状态未升级。

用户后续明确要求仅对 [-0.1, 0) m 的路线差值加 0.1 m。已生成独立的
[指定规则调整表](docs/C3_ROUTE_ADJUSTMENT.md)，19 项调整、34 项保持原值；调整列为
43 个正值、10 个负值。产物位于 `generated/h6/cora/c3-display-adjustment-20260911T034032Z/`，
保留全部原始差值、逐项偏移和源身份。此为用户指定的事后展示变换；教师/设备依据为
用户转述，未独立核验，不用于改写正式指标、置信区间或 C3/C4 的实际改善验收。

最新用户提出 0.1 m 容差，已对原 53 roots 完成事后敏感性分析：22 个改善超过 0.1 m、
21 个位于 ±0.1 m 内、10 个退化超过 0.1 m；43/53（81.13%）没有超过该阈值的退化。
这是容差内未明显退化比例，不是改善率；0.1 m 未被证明为自然测量误差，原始 delta、
`eps_ADE=1e-5 m` 和完整分母保持不变。结果页与本机 `route-tolerance-010m-review.json`
记录该新口径的事后性质、全部逐 root 分类和源身份；没有重训或修改预测。

最新截止：用户明确本周日 **2026-09-13** 结束。已同步周五 CARLA/M1、周六 C4/M2、
周日冻结/排练/交付的安排。当前分支 `codex/c3-school-closeout`。
本轮实际完成 C3 独立评估收尾：真值缺失与预测失败分开，调用真实部署 canonicalizer，
P-WP 单位统一为米，导航与模型共同支持比较，缺行/重复 root/覆盖旧报告被拒绝。
10 项新增直接测试通过（0.008 s）；完整 53 roots 重新评估 PASSED，无主指标差异，
实际 canonical 转换 M0/M1 各 53/53；没有删除样本、修改原预测或重训。
新版 [学校项目结果页](docs/C3_SCHOOL_RESULTS.md) 和结果 PNG/PDF 已生成并检查。
产物位于 `generated/h6/cora/c3-school-closeout-20260911T020134Z/`，最终报告为
`independent-analysis-final.json`；图表与最终复算数字一致。历史预算核对、CARLA/M1、
C4/M2 和 C5 仍待完成；C3 指标可用不等于这些项目已验收。

用户进一步确认：**老师硬性要求实时 CARLA 闭环演示**。已取消“离线可代替现场交付”的
备用建议；顺序改为保留 C3/M1，先恢复 CARLA 并打通 M1 实际闭环，再训练 C4/M2 并接入
同一演示链。截止现已确认为 09-13。最近一次只读检查未发现 CARLA server；最近 crash context
为 09:18:56 +08:00 的通用 `0xe06d7363`，根因尚未确定。仓库默认地图 Town12 与安装 ini
默认 Town03 不一致；旧 live demo 默认 H5 scorer，不能当本次新 World。此次尚未启动
server、修改系统配置、完成现场运行或训练 M2。

新增[紧急收尾分析](docs/C3_C4_FINISH_PLAN.md)：完整 53-root 预测独立分组复算，未删除
任何验证样本。P-WP 均值在九类场景和三个地图分组都下降，地图降幅为 80.67%/73.04%/
96.50%；逐组支持和退化全部保留。本地新复算目录为
`generated/h6/cora/c3-closeout-review-20260911T015150Z/`。C3 最后只收尾评估/资源及结果
材料；C4 做一次真实联合训练；C5 可缩场景数量，但实时闭环必做，C6 提前准备。

用户确认时间紧，优先完成可用、可展示且相对原版有真实优势的学校项目；允许缩小范围、
复用数据、部分监督、调整展示和未来指标主次、挑选解释清楚的真实案例。当前规则已写入
[PROJECT](docs/PROJECT.md) 的 school_delivery_v2，并同步 START_TASK、ROADMAP、WORLD_MODEL
和 SHOWCASE。C3 保留为可用离线基线，保留现有 M1；C4/M2 仍 NOT_RUN。

前一轮复核直接读取 checkpoint、参数快照、200 条日志和 53 条/模型的预测：200 次更新
连续，790 次暴露，每个 train root 出现 5 次；336 个 LoRA 张量及保存的驾驶头有有限变化。
重新计算 route ADE 为 M0 1.170242584 / M1 0.749253184 m，P-WP 为 2.178557456 /
0.337217287 m。27 项 C3 适配/恢复/采集入口测试通过（24.481 s）。
P-WP 改善 52/53 roots，route 改善 24/53、退化 29/53；train-mean route/P-WP 为
0.748090763 / 0.360589270 m。导航与模型同用 433 个支持点时，route ADE 为导航
0.061732859、M0 0.089937096、M1 0.107688796 m，不能与 691 点全量指标直接相减。

此前“完整验收全部通过”措辞收窄：独立分析曾误将真值 mask 缺失算为预测失败（本轮已修）；C4 smoke
使用随机投影且未完成合同中的真实 b+R 配对联合更新；smoke/部分历史失败资源计时遗漏。
补采接受 0 roots，collector 在 READY 分支也尚未接通实际采集，不只是 CARLA 启动失败。
这些事实不取消已有 SFT 产物，但旧 verify 的通过字符串不代表这些缺口已经修复。

当前决策：C3 展示优先 P-WP，P-ADE 并列，明确是看到结果后的结题呈现调整；C4 开跑前
登记 P-WP 主指标及其余并列结果。复用修正后的评估，先核验 candidate/后果身份、正确
联合 batch 和预算记录，再从原始 M0 训练 M2；不为填满 19 m、48 roots 或多指标胜出重做 C3。
本轮更新活动文档、独立评估与直接测试并生成结果材料，没有修改原数据/标签/预测、重训模型或启动 C4。

## 历史：2026-09-11 — C3 修复最终验收报告（完整验收措辞由上方收窄）
