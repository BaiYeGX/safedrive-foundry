...
跨阶段时间、身份、接口、Oracle/Observable、Profile、权限和Registry约束只在[执行架构](./docs/project/EXECUTION_ARCHITECTURE.md)维护；G2～G8 的 VLA/World/Safety 运行链、资源与降级边界见[工业化架构规范](./docs/project/G2_G8_INDUSTRIAL_ARCHITECTURE.md)；Windows CARLA 与 WSL 模型共享 4080 的准入、互斥 workload 和降级规则见[单机执行预算](./docs/project/SINGLE_MACHINE_EXECUTION_BUDGET.md)；通用完成规则只在`AGENTS.md`维护；最终研究主张见[CLAIMS](./docs/project/CLAIMS.md)。

两条学习主线不是两个孤立演示：VLA 从 G3 数据/基线/轻量模型/闭环进入 G5 World 排序和 G6 安全后训练；World Model 从 G5 动作分支数据/简单基线/结构化动力学进入候选排序、误排挖掘和 G8 消融。二者始终位于 Validator/Safety Kernel 约束内，完整任务链见 `START_TASK.md` 第 5 节。
...
| G4 | 5 | 场景注册、同预算搜索、反事实重建与最小化 | 失败有效、可解、可复现、可去重、可最小化 |
| G5 | 5 | 动作条件结构化世界模型、因果校准与主动验证 | 世界模型真实使用 Ego 动作并改善至少一个候选排序切片，失效可降级 |
| G6 | 5 | 事件数据、Shadow DAgger、偏好后训练与抗遗忘 | 完成一轮可信飞轮且正常/OOD/实时性不过线退化 |
...
| G4-04 | 可比重建、反事实分支、最小化与聚类 | G4-03 |
| G4-05 | 搜索、反事实与复现性阶段验收 | G4-01～G4-04 |

## 8. G5：动作条件交互世界模型

| ID | 能力包 | 直接依赖 |
|---|---|---|
| G5-01 | 同场景多动作分支数据与 CV/CTRV/IDM/Reward 基线 | G4-05 |
