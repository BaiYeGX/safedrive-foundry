#!/usr/bin/env python3
"""Materialize and collect H3v2 Challenge paired outcomes using the H2 contract."""
...
from data_pipeline.h3.challenge_matrix_v2 import (  # noqa: E402
    CHALLENGE_FIXED_MATRIX as FIXED_MATRIX,
    CHALLENGE_MATRIX_SHA256 as MATRIX_SHA256,
    CHALLENGE_PILOT_MATRIX as PILOT_MATRIX,
    materialize_challenge_physical_scenario as materialize_physical_scenario,
)
...
from data_pipeline.h3.contracts import H3_CONFIG_SHA256 as H2_CONFIG_SHA256  # noqa: E402
from data_pipeline.h2.config import config_identity  # noqa: E402
...
DATA_ROOT = ROOT / "generated" / "h3" / "carla-challenge-v2"
...
def materialize_map(dataset_id: str, map_name: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "schema_version": "safedrive.h3.challenge.physical_manifest_part.v1",
...
def freeze_manifest(dataset_id: str) -> dict[str, Any]:
    ...
    if len(rows) != 96 or len({row["pair_id"] for row in rows}) != 96:
        raise RuntimeError("PHYSICAL_MANIFEST_NOT_96_UNIQUE")
    worktrees = {
        (
    ...
    payload: dict[str, Any] = {
        "schema_version": "safedrive.h3.challenge.physical_manifest.v1",
        "dataset_id": dataset_id,
        "matrix_sha256": MATRIX_SHA256,
        "config_sha256": H2_CONFIG_SHA256,
        "config": config_identity(),
...
def restart_smoke(dataset_id: str, map_name: str) -> dict[str, Any]:
    ...
    physical = materialize_physical_scenario(
        ...
        next(
            ...
            and row.scenario.family == "emergency_lead_brake"
    ...
    _require_clean_scene(world)
    payload: dict[str, Any] = {
        "schema_version": "safedrive.h3.challenge.restart_smoke.v1",
        "dataset_id": dataset_id,
        "map_name": map_name,
        "connection": report.to_dict(),
        "config": config_identity(),
...
def _load_physical_manifest(store: PairedOutcomeStore) -> tuple[dict[str, PhysicalScenario], dict[str, Any]]:
    ...
    if os.environ.get("SDF_ALLOW_WORKTREE_CHANGE") not in {"1", "true", "yes"}:
...
def _npc_controls(scenario: PhysicalScenario, *, tick_index: int | None = None, pre_roll: bool = False) -> dict[str, Any]:
    """Return NPC controls with optional pre-roll / delayed-event phases."""
...
def _runtime(
    ...
    *,
    ...
    identity = RunIdentity(
        experiment_id="h3-challenge-paired-outcomes",
        run_id=run_id,
...
def _branch(
    ...
    try:
        ...
        candidate_set = PolicyCandidateSet(
            ...
            model_id="h3-forced-single-candidate@1.0.0",
            carla_frame=int(header.carla_frame),
            simulation_time_s=float(header.simulation_time),
        ...
        availability = ComponentAvailability(
            ...
            detail={"world": "H3_OFFLINE_COLLECTION", "forced_candidate_id": candidate.candidate_id},
        )
        safety = SafetyKernel()
        ...
        if not reset_cmp.comparable:
            ...
            red_after_tick = int(scenario.script.get("red_after_tick", -1))
            for tick in range(50):
                now_s = float(current_header.simulation_time)
                if red_after_tick >= 0 and tick == red_after_tick:
                    lights.force_red()
                if tick == 0:
                    command = applied
                else:
...
def _collect_map_impl(
    ...
    *,
    ...
    store.write_manifest()
    payload = {
        "schema_version": "safedrive.h3.challenge.collect_map_evidence.v1",
        "dataset_id": dataset_id,
        "map_name": map_name,
        "scope": scope,
        "connection": report.to_dict(),
...
def collect_map(dataset_id: str, map_name: str, scope: str) -> dict[str, Any]:
    ...
    try:
        ...
        _atomic_json(
            ...
            {
                "schema_version": "safedrive.h3.challenge.collect_map_failure.v1",
                "dataset_id": dataset_id,
                "map_name": map_name,
                "scope": scope,
                "config": config_identity(),
